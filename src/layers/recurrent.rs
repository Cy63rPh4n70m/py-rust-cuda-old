use ndarray::{s, Array2, ArrayD, ArrayView2, ArrayViewMut2};
use rand::Rng;
...
pub struct RecurrentDense
{
    ...
    // history vector
    /*
    pub input_vec_hist: Vec<Array2<f32>>,
    pub input_middle_dot_hist: Vec<Array2<f32>>,
    pub input_middle_dot_silu_hist: Vec<Array2<f32>>,

    pub hidden_middle_dot_hist: Vec<Array2<f32>>,
    pub hidden_middle_dot_silu_hist: Vec<Array2<f32>>,

    pub combined_hist: Vec<Array2<f32>>,
    pub combined_dot_hist: Vec<Array2<f32>>,
    pub combined_dot_silu_hist: Vec<Array2<f32>>,

    pub output_dot_hist: Vec<Array2<f32>>,
    pub output_dot_tanh_hist: Vec<Array2<f32>>,

    pub prev_hidden_state_hist: Vec<Array2<f32>>,
    */

    // layer weight definitions
    pub input_weights2d: Array2<f32>,
    pub input_weights2d_grads: Array2<f32>,
    pub input_biases: Array2<f32>,
    pub input_biases_grads: Array2<f32>,
    
    pub hidden_weights2d: Array2<f32>,
    pub hidden_weights2d_grads: Array2<f32>,
    pub hidden_biases: Array2<f32>,
    pub hidden_biases_grads: Array2<f32>,
    
    // silu activated
    pub combine_weights2d: Array2<f32>,
    pub combine_weights2d_grads: Array2<f32>,
    pub combine_biases: Array2<f32>,
    pub combine_biases_grads: Array2<f32>,
    
    // tanh activated
    pub output_weights2d: Array2<f32>,
    pub output_weights2d_grads: Array2<f32>,
    pub output_biases: Array2<f32>,
    pub output_biases_grads: Array2<f32>,
}

impl RecurrentDense {
    pub fn new(hidden_state_len: usize, middle_len: usize, return_sequence: bool, lr: f32) -> Self
    {
        return Self
        {
            in_shape: 0,
    ...
    pub fn forward(&mut self, input_seq: ArrayD<f32>) -> ArrayD<f32>
    {
        if self.in_shape == 0
        {
            ...
            let input_weights_range: f32 = (6.0 / (self.in_shape + self.middle_len) as f32).sqrt();
            let hidden_weights_range: f32 = (6.0 / (self.hidden_out_shape + self.middle_len) as f32).sqrt();
            let combine_weights_range: f32 = (6.0 / (self.middle_len + self.hidden_out_shape) as f32).sqrt();
            let output_weights_range: f32 = (6.0 / (self.hidden_out_shape + self.hidden_out_shape) as f32).sqrt();
        ...
        let input_seq: Array2<f32> = input_seq.into_shape((self.seq_len, self.in_shape)).unwrap();
        
        // output sequence (hidden states)
        let mut output_seq: Array2<f32> = Array2::zeros((self.seq_len, self.hidden_out_shape));
        ...
        let mut current_hidden_state: Array2<f32> = Array2::zeros((1, self.hidden_out_shape));
        ...
        for i in 0..self.seq_len
        {
            ...
            let input_slice: ArrayView2<f32> = 
                self.input_vec_seq.slice(s![i..i + 1, ..]);
            
            // calculate result from middle
            let input_middle_dot: Array2<f32> = input_slice.dot(&self.input_weights2d) + &self.input_biases;
            let input_middle_dot_silu: Array2<f32> = input_middle_dot.mapv(|v: f32| silu(v));
            let hidden_middle_dot: Array2<f32> = current_hidden_state.dot(&self.hidden_weights2d) + &self.hidden_biases;
            let hidden_middle_dot_silu: Array2<f32> = hidden_middle_dot.mapv(|v: f32| silu(v));

            // calculate result of combined
            let combined: Array2<f32> = &input_middle_dot_silu + &hidden_middle_dot_silu;
            let combined_dot: Array2<f32> = combined.dot(&self.combine_weights2d) + &self.combine_biases;
            let mut combined_dot_silu: Array2<f32> = combined_dot.mapv(|v: f32| silu(v));

            // residual connection
            combined_dot_silu += &current_hidden_state;

            // convert combined to tanh (output)
            let output_dot: Array2<f32> = combined_dot_silu.dot(&self.output_weights2d) + &self.output_biases;
            let output_dot_tanh: Array2<f32> = output_dot.mapv(|v: f32| tanh(v)); // next hidden state
            ...
            let mut output_seq_slice: ArrayViewMut2<f32> = output_seq.slice_mut(s![i..i + 1, ..]);
        ...
        {
            let output_seq_last_slice: ArrayView2<f32> = 
                output_seq.slice(
    ...
    pub fn backward(&mut self, loss_r_outputs: ArrayD<f32>) -> ArrayD<f32>
    {
        ...
        let mut loss_r_inputs: Array2<f32> = Array2::zeros((cols, rows));
        //let mut main_grads: Array2<f32> = loss_r_outputs.clone();
        ...
        for i in (0..self.timestep_hist.len()).rev()
        {
            ...
            {
                let loss_r_output_slice: ArrayView2<f32> = loss_r_outputs.slice(s![i..i + 1, ..]);
            ...
            let residual_grads: Array2<f32> = main_grads.clone();
            //println!("1");
            ...
            let main_grads_input: Array2<f32> = main_grads.clone();

            // backpropagate through input section
            let input_grads: Array2<f32> = backward_through_layer(
            ...
            //println!("loop: {}", i);
            let mut loss_r_inputs_slice: ArrayViewMut2<f32> = loss_r_inputs.slice_mut(s![i..i + 1, ..]);
...
fn backward_through_layer(
    mut main_grads: Array2<f32>, 
    act_deriv_fn: fn(f32) -> f32, input_dot: &Array2<f32>, // for activation layer
    weight_input: &Array2<f32>, weights2d: &Array2<f32>, // for weight layer
    weights2d_grads: &mut Array2<f32>, biases_grads: &mut Array2<f32>, // to update
) -> Array2<f32>
{
    //////////////////////////////////////////////////////////////
    ...
    let act_grads: Array2<f32> = backward_for_act(
        act_deriv_fn, 
        input_dot);
    ...
    // x_dot = x * w
    // dx_dot/dw
    let weight_grads: Array2<f32> = backward_for_weights(&main_grads, weight_input);
...
fn backward_for_weights(
    grads: &Array2<f32>, input: &Array2<f32>) -> Array2<f32>
{
    let new_cols: usize = input.shape()[1];
    let new_rows: usize = grads.shape()[1];

    // transpose and broadcast input vector
    let input_t: ArrayView2<f32> = input.t();
    let input_t_broadcast: ArrayView2<f32> = input_t.broadcast((new_cols, new_rows)).unwrap();
...
fn backward_for_act(
    act_deriv_fn: fn(f32) -> f32, input_dot: &Array2<f32>) -> Array2<f32>
{
    let act_r_unact: Array2<f32> = input_dot.mapv(|v: f32| act_deriv_fn(v));
    // chain rule
    return act_r_unact;
}
...
{
    input_vec: Array2<f32>,
    input_middle_dot: Array2<f32>,
    input_middle_dot_silu: Array2<f32>,
    hidden_middle_dot: Array2<f32>,
    hidden_middle_dot_silu: Array2<f32>,
    combined: Array2<f32>,
    combined_dot: Array2<f32>,
    combined_dot_silu: Array2<f32>,
    output_dot: Array2<f32>,
    output_dot_tanh: Array2<f32>,
    prev_hidden_state: Array2<f32>,
}
impl TimeStepState
{
    fn new(
        input_vec: Array2<f32>, input_middle_dot: Array2<f32>, input_middle_dot_silu: Array2<f32>,
        hidden_middle_dot: Array2<f32>, hidden_middle_dot_silu: Array2<f32>,
        combined: Array2<f32>, combined_dot: Array2<f32>, combined_dot_silu: Array2<f32>,
        output_dot: Array2<f32>, output_dot_tanh: Array2<f32>, prev_hidden_state: Array2<f32>
    ) -> Self
    {
        return Self
        {
