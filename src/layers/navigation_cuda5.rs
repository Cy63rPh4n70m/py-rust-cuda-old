...
use super::{activation::Activation, activation_cuda::ActivationCuda, broadcast_cuda::BroadcastMulCuda, broadcast_inv_cuda::BroadcastInvMulCuda, dense_cuda::DenseCuda, elementwise_cuda::ScalingCuda, l2norm_cuda::L2NormCuda, layer_norm_cuda::LayerNormCuda, softmax_cuda::SoftmaxCuda};
...
impl Navigation
{
        ...
        ) -> Self
    {
        ...
        let navigator_pre_scaler_layer: ScalingCuda = ScalingCuda::new(
            (6.0 / (n_heads as f32)).sqrt(), 2, lr
        );
        let mut navigator_layer: DenseCuda = DenseCuda::new(latent_embedding_len, l2, lr, true);
        ...
        let navigator_scaler_layer: ScalingCuda = ScalingCuda::new(
            (6.0 / (seq_len as f32 + 1.0)).sqrt(), 2, lr
        );
        ...
        let mut latent_modifier_layer: DenseCuda = DenseCuda::new(latent_embedding_len, l2, lr, true);
        latent_modifier_layer.use_bias = false;
        let latent_modifier_act_layer: ActivationCuda = ActivationCuda::new("silu", lr);
        let latent_modifier_norm_layer: LayerNormCuda = LayerNormCuda::new(lr, 1);
        ...
        let mut input_resizer_layer: DenseCuda = DenseCuda::new(latent_embedding_len, l2, lr, true);
        input_resizer_layer.use_bias = false;
        let input_resizer_act_layer: ActivationCuda = ActivationCuda::new("tanh", lr);
        let input_resizer_norm_layer: LayerNormCuda = LayerNormCuda::new(lr, 2);

        let scaler_layer: ScalingCuda = ScalingCuda::new(1.0 / (seq_len as f32).sqrt(), 2, lr);
        ...
        let aggregator_act_layer: ActivationCuda = ActivationCuda::new("tanh", lr);
        let mut aggregator_resizer_layer: DenseCuda = DenseCuda::new(input_embedding_len, l2, lr, true);
        ...
        let mut value_layer: DenseCuda = DenseCuda::new(input_embedding_len, l2, lr, true);
        value_layer.use_bias = false;
        let value_layer1: DenseCuda = DenseCuda::new(latent_embedding_len, l2, lr, true);
        let value_act_layer: ActivationCuda = ActivationCuda::new("tanh", lr);
        let value_norm_layer: LayerNormCuda = LayerNormCuda::new(lr, 1);

        let aggregator_scaler_layer: ScalingCuda = ScalingCuda::new((6.0 / (n_heads as f32)).sqrt(), 2, lr);
        //let value_softmax: SoftmaxCuda = SoftmaxCuda::new();

        //let final_act_layer: ActivationCuda = ActivationCuda::new("silu", lr);
        
        let latent_norm_layer: LayerNormCuda = LayerNormCuda::new(lr, 2);
        let latent_act_layer: ActivationCuda = ActivationCuda::new("tanh", lr);
        let latent_broadcast_layer: BroadcastMulCuda = BroadcastMulCuda::new(
            (n_heads, latent_embedding_len, input_embedding_len), 2, lr
        );
        let output_embedding_norm_layer: LayerNormCuda = LayerNormCuda::new(lr, 2);
        
        let extra_dense_layer: DenseCuda = DenseCuda::new(latent_embedding_len, l2, lr, true);
        let extra_dense_layer1: DenseCuda = DenseCuda::new(input_embedding_len, l2, lr, true);
        let extra_dense_layer2: DenseCuda = DenseCuda::new(input_embedding_len, l2, lr, true);
        let extra_act_layer: ActivationCuda = ActivationCuda::new("silu", lr);
        let extra_act_layer1: ActivationCuda = ActivationCuda::new("silu", lr);
        ...
        return Self
        {
            ...
            latent_resized_ptr_t: "".to_string(),
    ...
    pub fn initialize_ptrs(&mut self)
    {
        ...
        self.latent_resized_ptr_t = new_cuda_ptr_str(&[self.n_heads, self.latent_embedding_len, 1]);
        ...
        self.head_output_sum_ptr = new_cuda_ptr_str(&[1, self.seq_len, self.input_embedding_len]);
    ...
    pub fn forward(&mut self, input_embeddings_struct: &mut CudaTensorPtr)
    {
        ...
        let latent_resized_ptr_t: *mut f32 = string_to_ptr(&self.latent_resized_ptr_t);
        ...
        let head_output_sum_ptr: *mut f32 = string_to_ptr(&self.head_output_sum_ptr);
        ...
        */
        ...
        //);
        self.navigator_scaler_layer.forward(&mut navigator_ptr_struct);
        //self.navigator_act_layer.forward(&mut navigator_ptr_struct);
        //println!("query_norm: {:?}", cuda_ptr_to_array(string_to_ptr(&self.navigator_scaler_layer.weight_ptr), navigator_ptr_struct.get_shape()));
        //exit(1);
        //println!("query_norm: {:?}", cuda_ptr_to_array(navigator_ptr_struct.get_ptr(), navigator_ptr_struct.get_shape()));
        //exit(1);
        ...
        transpose_2d(latent_resized_ptr_t, navigator_ptr_struct.get_ptr(), self.n_heads, 1, self.latent_embedding_len);
        let mut val_ptr_struct: CudaTensorPtr = CudaTensorPtr {
            tensor_ptr: ptr_to_string(latent_resized_ptr_t),
        ...
        */
        ...
        //self.aggregator_scaler_layer.forward(&mut aggregator_ptr_struct);
        ...
        sum_axis(
            head_output_sum_ptr, aggregator_ptr_struct.get_ptr(), 
            self.n_heads, self.seq_len, self.input_embedding_len, 
            0, true
        );
        let mut head_output_sum_struct: CudaTensorPtr = CudaTensorPtr
        {
            tensor_ptr: self.head_output_sum_ptr.clone(),
            shape: vec![1, self.seq_len, self.input_embedding_len]
        };

        self.aggregator_act_layer.forward(&mut head_output_sum_struct);
        
        sum_axis(
            output_embedding_ptr, head_output_sum_struct.get_ptr(), 
        ...
        //let input_embeddings_struct_cpy: CudaTensorPtr = input_embeddings_struct.clone();
        ...
        //scalar_op_3d_inplace(
        //    input_embeddings_struct.get_ptr(), 
        //    (self.n_heads as f32 + 1.0).sqrt(), 3, 
        //    1, self.seq_len, self.input_embedding_len
        //);
    ...
    pub fn backward(&mut self, grads: &mut CudaTensorPtr)
    {
        ...
        */
        ...
        self.navigator_scaler_layer.backward(&mut latent_space_struct);
