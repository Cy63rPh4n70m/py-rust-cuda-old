...
class Trainer:
    ...
    def calc_ce_loss(self, output_id: str, target_classes: np.ndarray, batch: int, rows: int, cols: int):
        '''
    ...
    def update_params(self, layer_id: str, optimizer: str, lr: float, l2: float, alpha: float = 0.9, beta: float = 0.999):
        if optimizer.lower() == "sgd":
            ...
            print(f"Error: Optimizer {optimizer} not allowed, (sgd or adamw)")
            raise SystemExit()
    ...
    def calc_loss_and_grads(self, loss_type: str, pred: np.ndarray, actual: np.ndarray, flattened_len: int) -> tuple[float, np.ndarray]:
        ...
        self.lib_funcs.get("free_raw_vector")(loss_grads_ptr, flattened_len)
